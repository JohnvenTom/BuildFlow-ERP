  ...
  <div class="purchase-return-container">
    ...
    >
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="120px">
        ...
        <el-form-item label="关联入库单" prop="stockInId">
          <el-select v-model="form.stockInId" placeholder="请选择关联入库单（可选）" clearable style="width: 100%">
            <el-option
...
import { purchaseReturnPage, purchaseReturnAdd, purchaseReturnAudit, purchaseReturnVoid, stockInPage } from '@/api/purchase'
...
const form = reactive({
  supplierId: undefined as number | undefined,
  stockInId: undefined as number | undefined,
  remark: '',
...
async function loadData() {
  loading.value = true
  try {
    const res = await purchaseReturnPage({
...
async function handleAudit(row: any) {
  ...
  await purchaseReturnAudit(row.id)
...
async function handleVoid(row: any) {
  ...
  await purchaseReturnVoid(row.id)
