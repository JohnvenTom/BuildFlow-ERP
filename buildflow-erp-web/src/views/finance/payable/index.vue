  ...
  <div class="payable-container">
      ...
      <el-row :gutter="16">
        ...
        <el-col :span="8">
          <div class="summary-item">
            ...
            <div class="summary-value paid">{{ formatAmount(summary.paidAmount) }}</div>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="summary-item">
    ...
    <el-card shadow="never" class="table-card">
      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="payableAmount" label="应付金额" width="130" show-overflow-tooltip>
          <template #default="{ row }">{{ formatAmount(row.payableAmount) }}</template>
        </el-table-column>
        <el-table-column prop="paidAmount" label="已付金额" width="130" show-overflow-tooltip>
...
import { payablePage, payableSummary } from '@/api/finance'
...
/**
 * 加载应付台账列表数据
 * @description 根据搜索条件和分页参数请求应付台账分页数据
 */
async function loadData() {
  loading.value = true
  try {
    const res = await payablePage({
...
/**
 * 加载汇总数据
 * @description 请求应付台账的汇总金额信息
 */
async function loadSummary() {
  const res = await payableSummary()
